...
class CRSNet(object): #ClsRegSegNet
	...
	def replay_Q(self, batch_size, GAMMA=.99):
		#batch, _, ixs = self.q_buff.sample(g_step)
		batch = self.q_buff.sample(batch_size)
	...
	def train(self, dqn_generator, verbose=False):
		...
		for i in range(episode_count):
			...
			for j in range(max_steps):
				...
				self.replay_Q(BATCH_SIZE) #g_step
