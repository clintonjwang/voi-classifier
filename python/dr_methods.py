...
def load_vois_batch(cls=None, voi_dfs=None, acc_nums=None, verbose=False, overwrite=True):
	...
	if cls is None:
		for cls in C.classes_to_include:
			load_vois_batch(cls, voi_dfs, acc_nums, verbose, overwrite)
...
def load_imgs_batch(acc_nums, cls, update_intensities=False):
	...
	if cls=="hcc":
		try:
			dims_df = _load_imgs("Z:\\" + C.img_dirs[index], cls, acc_nums=acc_nums)
		except:
			pass
		try:
			dims_df = _load_imgs("Z:\\optn5b", cls, acc_nums=acc_nums)
		except:
			pass
	else:
		dims_df = _load_imgs("Z:\\" + C.img_dirs[index], cls, acc_nums=acc_nums)
...
def _get_intensities(acc_num=None, cls=None):
...
def _load_imgs(img_dir, cls, dims_df=None, C=None, verbose=False, acc_nums=None):
	...
	for cnt, acc_num in enumerate(acc_nums):
		...
		subdir = img_dir+"\\"+acc_num
		art, cur_dims = hf.dcm_load(os.path.join(subdir, "T1_AP"), flip_x=True)
		ven, _ = hf.dcm_load(os.path.join(subdir, "T1_VP"), flip_x=True)
		...
		if C.nb_channels == 3:
			eq, _ = hf.dcm_load(os.path.join(subdir, "T1_EQ"), flip_x=True)
