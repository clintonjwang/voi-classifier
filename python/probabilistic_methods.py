...
def augment(lesion_id, model, voi_df, aug_factor=100):
...
def shift_voi(lesion_id, model, voi_df, small_voi_df, shift_combinations=None):
...
def mc_dropout(model, img, T=50):
	img = np.expand_dims(img, 0)
    logits = np.empty(T)
    uncertainties = np.empty(T)
    for ix in range(T):
        logits[ix], uncertainties[ix] = model.predict(img)
