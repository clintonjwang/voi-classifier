	...
	def rpn_loss_regr_fixed_num(y_true, y_pred):
		...
		return lambda_rpn_regr * K.sum(
			y_true[:, :, :, :, :(dims*2) * num_anchors] * (x_bool * (0.5 * x * x) + (1 - x_bool) *
				(x_abs - 0.5))) / K.sum(epsilon + y_true[:, :, :, :, :(dims*2) * num_anchors]) #(dims+2) * num_anchors
...
def class_loss_regr(num_classes):
	def class_loss_regr_fixed_num(y_true, y_pred):
		...
		return lambda_cls_regr * K.sum(y_true[:, :, :(dims+2)*num_classes] *
			(x_bool * (0.5 * x * x) + (1 - x_bool) *
			(x_abs - 0.5))) / K.sum(epsilon + y_true[:, :, :(dims+2)*num_classes])
