...
def calc_rpn(C, img_data, width, height, depth, resized_width, resized_height, resized_d, img_length_calc_function):
	...
	(output_width, output_height, output_depth) = img_length_calc_function(resized_width, resized_height, resized_d)
				...
				for jy in range(output_height):
					...
					for iz in range(output_depth):
						...
						if bbox_type == 'neg':
							...
							y_rpn_regr[jy, ix, iz, start:start+6] = best_regr
	...
	for idx in range(num_anchors_for_bbox.shape[0]):
		if num_anchors_for_bbox[idx] == 0:
			...
			start = 6 * (best_anchor_for_bbox[idx,3] + n_anchratios * best_anchor_for_bbox[idx,4])
			y_rpn_regr[
				best_anchor_for_bbox[idx,0], best_anchor_for_bbox[idx,1], best_anchor_for_bbox[idx,2],
				start:start+6] = best_dx_for_bbox[idx, :]
...
def get_anchor_gt(all_img_data, class_count, C, img_length_calc_function, backend, mode='train'):
	...
	while True:
		...
		for img_data in all_img_data:
			...
			(width, height, depth) = (img_data_aug['width'], img_data_aug['height'], img_data_aug['d'])
			...
			try:
				y_rpn_cls, y_rpn_regr = calc_rpn(C, img_data_aug, width, height, depth,
					resized_width, resized_height, resized_depth, img_length_calc_function)
			except:
				print("Exception in calc_rpn")
				continue
