...
def build_cnn(C, optimizer='adam', inputs=4, batch_norm=True, dilation_rate=(2, 2, 1)):
    ...
    if batch_norm:
        ...
        x = Dense(100)(intermed)#, kernel_initializer='normal', kernel_regularizer=l2(.01), kernel_constraint=max_norm(3.))(x)
        x = BatchNormalization()(x)
        x = ELU(alpha=1.0)(x)
        #x = Activation('relu')(x)
